...
in -- straight out of GarageBand, Voice Memos, a phone recording,
whatever -- there's no need to convert anything, just drop the files
in as-is.

If you ever do need to convert something TO m4a (or to mp3, wav,
etc.), the free, no-install-needed option is an online converter --
search "convert to m4a" and any of the common ones (e.g.
cloudconvert.com) will do it in a browser. If you'd rather have a
proper desktop tool: ffmpeg (free, command-line -- ffmpeg -i input.mp3
-c:a aac -b:a 192k output.m4a) or Audacity (free, has a simple
Export menu) both handle it well. None of this is needed unless you
end up with files in some other format first.
...
interaction) -- but that changes the intro from fully automatic to a
one-tap start, so it's your call whether that trade-off's worth it.
